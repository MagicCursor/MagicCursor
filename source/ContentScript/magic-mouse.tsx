...
export default function SplashCursor({
  ...
  useEffect(() => {
    ...
    function createProgram(
      ...
      // Ensure attribute location 0 maps to aPosition
    ...
    const blit = ((): ((target: FBO | null, doClear?: boolean) => void) => {
      ...
      return (target: FBO | null, doClear = false): void => {
        if (!gl) return;
        if (!target) {
    ...
    function correctRadius(radiusValue: number): number {
      const aspectRatio = canvas!.width / canvas!.height;
      let correctedRadius = radiusValue;
      if (aspectRatio > 1) correctedRadius *= aspectRatio;
    ...
    function correctDeltaX(deltaValue: number): number {
      const aspectRatio = canvas!.width / canvas!.height;
      let correctedDelta = deltaValue;
      if (aspectRatio < 1) correctedDelta *= aspectRatio;
      return correctedDelta;
    }

    function correctDeltaY(deltaValue: number): number {
      const aspectRatio = canvas!.width / canvas!.height;
      let correctedDelta = deltaValue;
      if (aspectRatio > 1) correctedDelta /= aspectRatio;
      return correctedDelta;
    ...
    const onVisibility = (): void => {
      // Reset timing to avoid large dt spikes after visibility change
